...
public class RuleTypeDTOTest extends BaseModuleContextSensitiveTest {
	...
	public void testMapRuleType() throws Exception {
		...
		Assert.assertNotNull(ruleTypeDTO);
		Assert.assertEquals(new Integer(1), ruleTypeDTO.getRuleTypeId());
		Assert.assertEquals("PWS", ruleTypeDTO.getName());
		Assert.assertEquals(new Integer(1), ruleTypeDTO.getCreator());
		Assert.assertEquals("2017-07-19 11:32:21", dt.format(ruleTypeDTO.getDateCreated()));
		Assert.assertEquals(Boolean.FALSE, ruleTypeDTO.getRetired());
		Assert.assertEquals("7cb211ff-6c97-11e7-9be2-0a0027000019", ruleTypeDTO.getUuid());
	}
}
